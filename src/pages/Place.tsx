...
import { sortHospitalsByStatusAndDistance } from './health/HealthHospitalData'
...
function Place() {
  ...
  const sortedHospitals = sortHospitalsByStatusAndDistance(
    placeHospitals.map((hospital, index) => ({ hospital, image: hospitalImages[index % hospitalImages.length] })),
    ({ hospital }) => ({
